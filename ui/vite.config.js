...
export default defineConfig(({ command, mode }) => {
    ...
    return {
      ...
      server: {
        proxy: {
          ...
          // unauthenticated in Polarion (see the requirements-inspector-app web.xml), so the dev proxy can fetch
          // it without a session.
          '/polarion/requirements-inspector-app/ui/generic': {
            target: polarionUrl,
            changeOrigin: true,
          },
          // The extension's own webapp context: its REST API, which the About page reads.
          '/polarion/requirements-inspector/rest': {
            target: polarionUrl,
            changeOrigin: true,
          },
          '/polarion/rest': {
  ...
  return {
    ...
    base: '/polarion/requirements-inspector-app/ui/app/',
    build: {
      outDir: './dist/app',
      emptyOutDir: true,
    },
